...
def determine_frame_skip(video_path):
    ...
    if duration <= 300:  # Video length <= 5 minutes
        ...
        return 30
    elif duration <= 7200:  # Video length > 1 hour
        return 60
    else:
        return 120
...
for idx, row in tqdm(metadata.iterrows(), total=len(metadata), desc="Processing Videos"):
    ...
    try:
        ...
        output_file = os.path.join(normal_output_dir, f"{video_name}_features.pt")

        # Skip processing if the file already exists
        if os.path.exists(output_file):
            tqdm.write(f"Skipping already processed video: {video_path}")
            continue
